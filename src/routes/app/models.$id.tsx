...
function EditModelPage() {
  ...
  if (!creator) return <main className="min-h-full bg-muted/20 p-4 md:p-8"><div className="mx-auto max-w-6xl space-y-4"><p className="text-sm text-destructive">{error || 'Creator not found.'}</p><Link to="/app/models" search={{ edit: undefined, new: undefined, search: undefined, status: undefined, queue: undefined }} className="inline-flex items-center gap-2 text-sm text-primary"><ArrowLeft className="h-4 w-4" />Back to creators</Link></div></main>

  return <main className="min-h-full bg-muted/20 p-4 md:p-8"><div className="mx-auto max-w-6xl space-y-6">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><Link to="/app/models" search={{ edit: undefined, new: undefined, search: undefined, status: undefined, queue: undefined }} className="mb-2 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" />Creators</Link><h1 className="text-2xl font-semibold tracking-tight">{creator.name}</h1><p className="mt-1 text-sm text-muted-foreground">Manage profile, posts, paid media and Reels from one workspace.</p></div><Link to="/creator/$slug" params={{ slug: creator.slug }} className="inline-flex h-10 items-center justify-center rounded-md border bg-background px-4 text-sm">View public profile</Link></header>
