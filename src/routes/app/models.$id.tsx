...
function EditModelPage() {
  ...
  useEffect(() => { void getAdminCreator(id).then(setCreator).catch((err) => setError(err instanceof Error ? err.message : 'Não foi possível carregar o modelo.')).finally(() => setLoading(false)) }, [id])
  return <main className="min-h-full bg-muted/20 p-4 md:p-8"><div className="mx-auto max-w-4xl space-y-6"><div><p className="text-sm text-muted-foreground">Administração / Modelos</p><h1 className="text-2xl font-semibold tracking-tight">Editar modelo</h1></div>{loading && <p className="text-sm text-muted-foreground">A carregar…</p>}{error && <div className="rounded-md border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">{error}</div>}{!loading && creator && <CreatorForm creator={creator} />}{!loading && !creator && !error && <p className="text-sm text-muted-foreground">Modelo não encontrado.</p>}</div></main>
}
