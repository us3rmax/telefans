...
function DashboardHome() {
  ...
  return <main className="min-h-full bg-muted/20 p-4 md:p-8"><div className="mx-auto max-w-7xl space-y-8">
    <header className="flex items-start justify-between gap-4"><div><p className="text-sm text-muted-foreground">TeleFans Admin</p><h1 className="text-2xl font-semibold tracking-tight">Dashboard</h1><p className="mt-1 text-sm text-muted-foreground">Creators, clients, and uploads in one place.</p></div><button type="button" onClick={() => void loadDashboard()} className="inline-flex h-9 items-center gap-2 rounded-md border bg-background px-3 text-sm hover:bg-muted"><RefreshCw className="h-4 w-4" />Refresh</button></header>
    ...
    {loading ? <p className="text-sm text-muted-foreground">Loading dashboard…</p> : <>
      <section className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border bg-background p-4 shadow-sm"><Users className="mb-3 h-5 w-5 text-primary" /><p className="text-2xl font-semibold">{clients.total}</p><p className="text-sm text-muted-foreground">Telegram users</p></div><div className="rounded-2xl border bg-background p-4 shadow-sm"><Clock3 className="mb-3 h-5 w-5 text-primary" /><p className="text-2xl font-semibold">{clients.recent.length}</p><p className="text-sm text-muted-foreground">Most recent users</p></div><div className="rounded-2xl border bg-background p-4 shadow-sm"><Activity className="mb-3 h-5 w-5 text-primary" /><p className="text-2xl font-semibold">{clients.active.filter(client => client.following_count > 0).length}</p><p className="text-sm text-muted-foreground">Users with follows</p></div></section>
      <section className="space-y-3"><div><h2 className="text-xl font-semibold">Users</h2><p className="text-sm text-muted-foreground">Persistent Telegram users, sorted by activity and signup date.</p></div><div className="grid gap-4 lg:grid-cols-3"><ClientSection title="Most recent" icon={Clock3} clients={clients.recent} empty="No registered users yet." /><ClientSection title="Most active" icon={Activity} clients={clients.active} empty="No following activity yet." /><ClientSection title="All users" icon={Users} clients={clients.all} empty="No users found." /></div></section>
      <section className="space-y-3"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-semibold">Creators</h2><p className="text-sm text-muted-foreground">Images become Paid Media; videos are added to Reels.</p></div><label className="grid gap-1 text-sm font-medium">Paid Media price<input type="number" min="0" step="1" value={unlockPrice} onChange={event => setUnlockPrice(event.target.value)} className="h-9 w-36 rounded-md border bg-background px-3" /><span className="text-xs font-normal text-muted-foreground">Coins per image</span></label></div><div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">{models.map((model) => { const busy = uploadingId === model.id; return <button key={model.id} type="button" disabled={busy} onClick={() => inputs.current[model.id]?.click()} className="group overflow-hidden rounded-2xl border bg-background text-left shadow-sm transition hover:-translate-y-0.5 hover:border-primary hover:shadow-md disabled:cursor-wait"><div className="relative aspect-[4/5] bg-muted"><img src={model.avatar_image || model.cover_image} alt={model.name} className="h-full w-full object-cover" /><div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-black/80 to-transparent px-3 pb-3 pt-10 text-white"><span className="truncate font-medium">{model.name}</span>{busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <ImagePlus className="h-5 w-5" />}</div></div><div className="flex items-center gap-2 p-3 text-sm text-muted-foreground"><Upload className="h-4 w-4" />Add media<input ref={element => { inputs.current[model.id] = element }} type="file" accept="image/*,video/*" multiple className="sr-only" onClick={event => event.stopPropagation()} onChange={event => { void handleFiles(model, Array.from(event.target.files ?? [])); event.currentTarget.value = '' }} /></div></button> })}{models.length === 0 && <div className="col-span-full rounded-xl border bg-background p-8 text-center text-sm text-muted-foreground">No creators have been added yet.</div>}</div></section>
    </>}
  </div></main>
}
