...
function DashboardHome() {
  ...
  return <main className="admin-dashboard"><div className="admin-dashboard-inner">
    ...
    {loading || !view ? <div className="admin-loading"><Loader2 className="h-5 w-5 animate-spin" />Loading CRM data…</div> : <>
      ...
      <section className="admin-panel"><div className="admin-panel-heading"><div><h2>Creator workspaces</h2><p>Open a creator workspace to edit the profile and manage Feed, Paid Media and Reels.</p></div><a href="/app/models" className="admin-inline-link">Open Creators →</a></div><div className="admin-two-column">{models.slice(0, 6).map(model => <a key={model.id} href={`/app/models?edit=${model.id}`} className="admin-list-row"><div className="admin-avatar">{model.avatar_image ? <img src={model.avatar_image} alt="" /> : model.name.slice(0, 1)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{model.name}</p><p className="truncate text-xs text-muted-foreground">@{model.handle.replace(/^@+/, '')}</p></div><span className="admin-inline-link">Manage →</span></a>)}{models.length === 0 && <div className="admin-empty-state">No creators available.</div>}</div></section>
    </>}
  </div></main>
}
