...
function AdminModelsPage() {
  ...
  return <main className="min-h-full bg-muted/20 p-4 md:p-8"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="overflow-hidden rounded-xl border bg-background shadow-sm"><div className="grid grid-cols-[1fr_auto] gap-4 border-b px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground md:grid-cols-[2fr_1fr_1fr_2fr]"><span>Creator</span><span className="hidden md:block">Status</span><span className="hidden md:block">Updated</span><span className="text-right">Workspace actions</span></div>
      ...
      {!loading && filtered.map(model => <div key={model.id} className="grid grid-cols-[1fr_auto] items-center gap-4 border-b px-4 py-4 last:border-b-0 md:grid-cols-[2fr_1fr_1fr_2fr]"><div className="flex min-w-0 items-center gap-3"><img src={model.avatar_image || '/placeholder-avatar.svg'} alt="" className="h-11 w-11 shrink-0 rounded-full object-cover" /><div className="min-w-0"><p className="truncate font-medium">{model.name}</p><p className="truncate text-sm text-muted-foreground">@{model.handle}</p></div></div><span className={`hidden w-fit rounded-full px-2 py-1 text-xs font-medium md:inline-flex ${model.published ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground'}`}>{model.published ? 'Published' : 'Draft'}</span><span className="hidden text-sm text-muted-foreground md:block">{new Date(model.updated_at).toLocaleDateString('en-US')}</span><div className="flex flex-wrap items-center justify-end gap-2"><Link to="/creator/$slug" params={{ slug: model.slug }} className="inline-flex items-center gap-1 rounded-md border px-2.5 py-1.5 text-xs font-medium hover:bg-muted"><Eye className="h-3.5 w-3.5" />View profile</Link><Link to="/app/models/$id" params={{ id: model.id }} className="inline-flex items-center gap-1 rounded-md border px-2.5 py-1.5 text-xs font-medium hover:bg-muted"><Pencil className="h-3.5 w-3.5" />Edit profile</Link><Link to="/app/content" className="inline-flex items-center gap-1 rounded-md border px-2.5 py-1.5 text-xs font-medium hover:bg-muted"><Files className="h-3.5 w-3.5" />Manage content</Link><button type="button" onClick={() => void togglePublished(model)} className={`inline-flex items-center gap-1 rounded-md border px-2.5 py-1.5 text-xs font-medium ${model.published ? 'border-emerald-500/30 text-emerald-700 hover:bg-emerald-50' : 'hover:bg-muted'}`}>{model.published ? <ToggleRight className="h-4 w-4" /> : <ToggleLeft className="h-4 w-4" />}{model.published ? 'Unpublish' : 'Publish'}</button></div></div>)}
