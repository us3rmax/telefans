...
function AdminModelsPage() {
  ...
  if (editId && !loading && editingCreator) return <main className="min-h-full bg-muted/20 p-4 md:p-8"><div className="mx-auto max-w-4xl space-y-6"><header className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Administration / Creators</p><h1 className="text-2xl font-semibold tracking-tight">Edit profile</h1><p className="mt-1 text-sm text-muted-foreground">Update {editingCreator.name} and save the changes directly to the creator record.</p></div><Link to="/app/models" search={{ edit: undefined, new: undefined }} className="rounded-md border px-3 py-2 text-sm">Back to creators</Link></header><CreatorForm creator={editingCreator} /></div></main>
