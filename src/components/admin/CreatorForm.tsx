...
export function CreatorForm({ creator }: Props) {
  ...
  return <form onSubmit={submit} className="space-y-6 rounded-xl border bg-background p-4 shadow-sm md:p-6">
    ...
    <div className="flex flex-wrap justify-end gap-2"><button type="button" onClick={() => void navigate({ to: '/app/models', search: { edit: undefined, new: undefined } })} className="h-10 rounded-md border px-4 text-sm">Cancel</button><button disabled={saving} className="h-10 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground disabled:opacity-60">{saving ? 'Saving…' : creator ? 'Save changes' : 'Create draft creator'}</button></div>
  </form>
}
