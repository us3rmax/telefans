...
const defaultSubscription: SubscriptionFormState = { plan_mode: 'free', title: 'Subscription', message: '', normal_price_stars: 0, promo_price_stars: 0, promo_days: 30, promo_expires_at: '', telegram_username: '', vip_channel_url: '', is_active: true }
...
function readSubscription(value: unknown): SubscriptionFormState {
  ...
  return {
    ...
    normal_price_stars: toNumber(raw.normal_price_stars, defaultSubscription.normal_price_stars),
    promo_price_stars: toNumber(raw.promo_price_stars, defaultSubscription.promo_price_stars),
...
function publicSubscription(value: SubscriptionFormState) {
  return {
...
export function CreatorForm({ creator }: Props) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const normalizedSubscription: SubscriptionFormState = {
        ...subscription,
        ...
        normal_price_stars: Math.max(0, Math.round(subscription.normal_price_stars)),
        promo_price_stars: Math.max(0, Math.round(subscription.promo_price_stars)),
      ...
      if (normalizedSubscription.plan_mode === 'paid' && normalizedSubscription.normal_price_stars < 1) throw new Error('A paid subscription needs a price of at least 1 Star.')
      if (normalizedSubscription.plan_mode === 'promo' && (normalizedSubscription.normal_price_stars < 1 || normalizedSubscription.promo_price_stars < 1)) throw new Error('A promotional subscription needs normal and promotional prices of at least 1 Star.')
      ...
      if (creator) {
        ...
        await upsertAdminSubscriptionSettings(creator.id, { ...normalizedSubscription, promo_expires_at: normalizedSubscription.promo_expires_at ? new Date(normalizedSubscription.promo_expires_at).toISOString() : null })
        ...
        await upsertAdminSubscriptionSettings(created.id, { ...normalizedSubscription, promo_expires_at: normalizedSubscription.promo_expires_at ? new Date(normalizedSubscription.promo_expires_at).toISOString() : null })
  ...
  return <form onSubmit={submit} className="space-y-6 rounded-xl border bg-background p-4 shadow-sm md:p-6">
    ...
    <section className="space-y-4 rounded-lg border bg-muted/10 p-4">
      <div><h2 className="font-semibold">Subscription offer</h2><p className="text-sm text-muted-foreground">Configure this creator independently. Paid offers use Telegram Stars and recurring plans renew every 30 days.</p></div>
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {isPaidOffer && <label className="space-y-1 text-sm"><span className="font-medium">Normal price (Stars)</span><input type="number" min="1" max="10000" step="1" value={subscription.normal_price_stars || ''} onChange={event => setSubscriptionValue('normal_price_stars', Number(event.target.value))} className="h-10 w-full rounded-md border bg-background px-3 outline-none focus:ring-2 focus:ring-primary/30" /></label>}
        {subscription.plan_mode === 'promo' && <><label className="space-y-1 text-sm"><span className="font-medium">Promotional price (Stars)</span><input type="number" min="1" max="10000" step="1" value={subscription.promo_price_stars || ''} onChange={event => setSubscriptionValue('promo_price_stars', Number(event.target.value))} className="h-10 w-full rounded-md border bg-background px-3 outline-none focus:ring-2 focus:ring-primary/30" /></label><label className="space-y-1 text-sm"><span className="font-medium">Promotion duration (days)</span><input type="number" min="1" max="3650" step="1" value={subscription.promo_days} onChange={event => setSubscriptionValue('promo_days', Number(event.target.value))} className="h-10 w-full rounded-md border bg-background px-3 outline-none focus:ring-2 focus:ring-primary/30" /></label><label className="space-y-1 text-sm"><span className="font-medium">Promotion ends (optional)</span><input type="datetime-local" value={subscription.promo_expires_at} onChange={event => setSubscriptionValue('promo_expires_at', event.target.value)} className="h-10 w-full rounded-md border bg-background px-3 outline-none focus:ring-2 focus:ring-primary/30" /></label></>}
