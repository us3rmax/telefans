...
import { createCarouselFromMediaAssets, deleteMediaAsset, listMediaAssets, setCreatorImageFromMediaAsset, setMediaAssetsPrice, uploadCreatorMediaBatch, type Creator, type CreatorImageField, type MediaAsset, type MediaBatchUploadProgress } from '@/lib/admin-repository'
...
const formatBytes = (bytes: number) => { if (!bytes) return '0 B'; const units = ['B', 'KB', 'MB', 'GB']; const index = Math.min(Math.floor(Math.log(bytes) / Math.log(1024)), units.length - 1); return `${(bytes / 1024 ** index).toFixed(index === 0 ? 0 : 1)} ${units[index]}` }

export function CreatorVault({ creatorId, creatorName, onCreatorUpdated }: Props) {
  const [assets, setAssets] = useState<MediaAsset[]>([])
  const [loading, setLoading] = useState(true)
  const [uploading, setUploading] = useState(false)
  const [uploadProgress, setUploadProgress] = useState<MediaBatchUploadProgress | null>(null)
  ...
  const closeImport = () => { if (uploading) return; setImportOpen(false); setPendingFiles([]); setUploadProgress(null) }

  const importFiles = async () => {
    if (!pendingFiles.length) return
    setUploading(true); setUploadProgress(null); setError(''); setMessage('')
    try {
      const results = await uploadCreatorMediaBatch(pendingFiles, creatorId, 10, false, progress => setUploadProgress(progress))
      ...
      setPendingFiles([]); setImportOpen(false); setUploadProgress(null)
